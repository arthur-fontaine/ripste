...
let viteServer: Awaited<ReturnType<typeof dev>>["viteServer"] | undefined =
	undefined;
...
beforeAll(async () => {
	...
	const { viteServer: _viteServer } = await dev({
		viteConfig: {
			...
			resolve: {
				alias: {
					"../../database.ts": path.resolve(
						root,
						"tests/test-utils/fakeDatabase.ts",
					),
					"../../apiClient.ts": path.resolve(
						root,
						"tests/test-utils/fakeApiClient.ts",
					),
				},
			},
		},
