...
function validateCompanyData(data: any): CreateCompanyRequest {
...
function validateUpdateCompanyData(data: any): UpdateCompanyRequest {
...
export const companyRouter = new Hono()
		...
		try {
			...
			if (error instanceof Error) {
				...
				if (error.message.includes("unique") || 
					...
					return c.json({ error: "A company with this KBIS already exists" }, 400);
				}
			}
	...
	.put("/:id", async (c) => {
		try {
			...
			if (error instanceof Error) {
				if (error.message.includes("cannot be") || error.message.includes("security reasons")) {
					return c.json({ error: error.message }, 400);
				}
			}
