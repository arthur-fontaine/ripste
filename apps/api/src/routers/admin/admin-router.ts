...
adminRouter.post("/send-email", async (c) => {
	try {
		const { to, subject, htmlContent, textContent } = await c.req.json();
...
adminRouter.post("/test-email", async (c) => {
	try {
		const { type, email, name } = await c.req.json();

		if (!type || !email) {
			return c.json(
				{
					error: "Les champs 'type' et 'email' sont requis",
				},
				400,
			);
		}

		const userName = name || "Utilisateur Test";
		...
		switch (type) {
			case "registration":
				await emailService.sendRegistrationConfirmation({
					userEmail: email,
					userName,
					confirmationUrl: `${process.env["FRONTEND_URL"]}/verify-email?token=test-token`,
				});
				break;
			...
			case "rejection":
				await emailService.sendPlatformRejection({
					userEmail: email,
					userName,
					reason: "Ceci est un test de refus",
					supportEmail: process.env["SUPPORT_EMAIL"] || "support@ripste.com",
				});
				break;

			default:
				return c.json({ error: "Type d'email non supporté" }, 400);
		}

		return c.json({
			message: `Email de test '${type}' envoyé avec succès`,
			email: email,
		});
	} catch (error) {
